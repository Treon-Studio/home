...
const MediaRenderer = memo(({ item, className, debug = false }: { item: CarouselItem | undefined; className?: string; debug?: boolean }) => {
  if (!item) return null;
  if (!debug) {
    if (item.type === 'custom') {
      return <div className={cn('h-full w-full', className)}>{item.content}</div>;
    }
    if (item.type === 'video') {
      return <video src={item.src} poster={item.poster} className={cn('h-full w-full object-cover', className)} muted loop autoPlay />;
    }
    return <img src={item.src} alt={item.alt || ''} draggable={false} className={cn('h-full w-full object-cover', className)} />;
  }

  return <div className={cn('flex h-full w-full items-center justify-center border text-2xl', className)}>{item.id}</div>;
});
...
const BoxCarousel = forwardRef<BoxCarouselRef, BoxCarouselProps>(
  (
    ...
    // Reset indices when items length changes (e.g. hot reload or prop change)
    ...
    if (prevItemsLength.current !== items.length) {
      ...
      setCurrentIndex(0);
      setNextIndex(1);
      setAfterNextIndex(Math.min(2, items.length - 1));
    }
    ...
    const goTo = useCallback(
      (targetIndex: number) => {
        if (items.length === 0 || isRotating.current || targetIndex === currentItemIndex) return;

        // Determine shortest path
        const forwardSteps = (targetIndex - currentItemIndex + items.length) % items.length;
        const backwardSteps = (currentItemIndex - targetIndex + items.length) % items.length;
        ...
        // Queue remaining steps
        const remainingSteps = Math.min(forwardSteps, backwardSteps) - 1;
        if (remainingSteps > 0) {
      ...
      [items.length, currentItemIndex, next, prev],
    );
    ...
    useImperativeHandle(
      ...
      [next, prev, goTo, currentItemIndex],
    );
    ...
    return (
      <div
        ...
        {...props}
      >
        <motion.div className="relative h-full w-full [transform-style:preserve-3d]" style={{ transform }}>
          <CubeFace transform={faceTransforms[0]} style={debug ? { width, height, backgroundColor: '#ff9999' } : { width, height }} debug={debug}>
            <MediaRenderer item={items[prevIndex]} debug={debug} />
          </CubeFace>
          <CubeFace transform={faceTransforms[1]} style={debug ? { width, height, backgroundColor: '#99ff99' } : { width, height }} debug={debug}>
            <MediaRenderer item={items[currentIndex]} debug={debug} />
          </CubeFace>
          <CubeFace transform={faceTransforms[2]} style={debug ? { width, height, backgroundColor: '#9999ff' } : { width, height }} debug={debug}>
            <MediaRenderer item={items[nextIndex]} debug={debug} />
          </CubeFace>
          <CubeFace transform={faceTransforms[3]} style={debug ? { width, height, backgroundColor: '#ffff99' } : { width, height }} debug={debug}>
